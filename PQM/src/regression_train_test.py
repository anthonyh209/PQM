...
def sum_of_least_squares_error(y_true, y_pred):
    """
    Parameters
    y_true - the true targets
    y_pred - the predicted targets
    ...
    mse - The root mean squared error between true and predicted target
    """
    mse = np.square(np.subtract(y_true, y_pred)).sum()
    return mse
...
def cv_evaluation_linear_model(
    ...
    for f,fold in enumerate(folds):
        ...
        # now train and evaluate the error on both sets
        train_error, test_error = train_and_test(train_inputs, train_targets, test_inputs, test_targets,reg_param=reg_param)
